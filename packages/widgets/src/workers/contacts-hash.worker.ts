...
import { BASE_SALT_FOR_CONTACTS, normalizeContactValue } from "../contacts"
...
export type ContactsHashWorkerOutput = {
  ...
  hashMap: Record<
    string,
    { name: string | undefined; value: string; photo?: string }
  >
}
...
self.onmessage = async (e: MessageEvent<ContactsHashWorkerInput>) => {
  ...
  for (const contact of contacts) {
    ...
    const photo =
      contact.photos?.find((p) => !p.default)?.url ?? contact.photos?.at(0)?.url
    const infos: string[] = [
