...
export class ScrollAreaElement extends LitElement {
  ...
  @state() declare private _thumbY: { top: number; height: number };
  @state() declare private _thumbX: { left: number; width: number };
  @state() declare private _hasScrollY: boolean;
  @state() declare private _hasScrollX: boolean;
  @state() declare private _dragging: 'y' | 'x' | null;

  @query('.viewport') declare private _viewport: HTMLElement;
  @query('.content') declare private _content: HTMLElement;
  ...
  private get _trackH() {
    return (this._viewport?.clientHeight ?? 0) - 4;
  }
  private get _trackW() {
    return (this._viewport?.clientWidth ?? 0) - 4;
  }
  ...
  firstUpdated() {
    ...
    this._viewport.addEventListener('scroll', () => this._sync(), {
      passive: true,
    });
    this._sync();
  }
  ...
  private _onThumbPointerDown(axis: 'y' | 'x', e: PointerEvent) {
    ...
    this._scrollStart =
      axis === 'y' ? this._viewport.scrollTop : this._viewport.scrollLeft;
  }
  ...
  private _onThumbPointerMove(axis: 'y' | 'x', e: PointerEvent) {
    ...
    const vp = this._viewport;
    const delta =
      axis === 'y' ? e.clientY - this._dragStart : e.clientX - this._dragStart;

    if (axis === 'y') {
      const scrollRatio =
        (vp.scrollHeight - vp.clientHeight) /
        (this._trackH - this._thumbY.height);
      vp.scrollTop = this._scrollStart + delta * scrollRatio;
    } else {
      const scrollRatio =
        (vp.scrollWidth - vp.clientWidth) / (this._trackW - this._thumbX.width);
