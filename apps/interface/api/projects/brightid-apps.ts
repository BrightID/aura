...
type BrightIdAppsResponse = Record<string, Record<string, unknown>>;
...
const handler = async (req: VercelRequest, res: VercelResponse) => {
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        isNotNull(projectsTable.brightIdAppId),
      ),
    )
  ...
  const apps = rows.reduce<BrightIdAppsResponse>((acc, row) => {
    const app = row.app;
    const images = parseTextArray(
      app.images,
      row.projectLogoUrl ?? row.projectImage,
    );
    ...
    setIfPresent(
      output,
      'verificationExpirationLength',
      app.verificationExpirationLength,
    );
    ...
    acc[row.projectId.toString()] = output;
    return acc;
  }, {});
