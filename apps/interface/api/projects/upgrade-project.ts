...
async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const { planId, projectId, orderId, isYearly } = upgradeSchema.parse(req.body)
    ...
    const [project, plan, payment] = await Promise.all([
      ...
      db
        ...
        .where(and(eq(verificationPlansTable.id, planId), eq(verificationPlansTable.isActive, true)))
        .limit(1),
      db
    ...
    if (payment[0].status !== 'finished')
      return res.status(402).json({ error: 'Payment not completed', status: payment[0].status })
