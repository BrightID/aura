...
export function computeRequirements(
  ...
  const lowFromL1 = auraImpacts.some(
    (i) => (i.level ?? 0) >= 1 && (i.confidence ?? 0) >= 1,
  );
  const mediumFromL1 = auraImpacts.some(
    (i) => (i.level ?? 0) >= 1 && (i.confidence ?? 0) >= 2,
  );
  const highFromL2 = auraImpacts.some(
    (i) => (i.level ?? 0) >= 2 && (i.confidence ?? 0) >= 3,
  );
  const mediumFromL2Count = auraImpacts.filter(
    (i) => (i.level ?? 0) >= 2 && (i.confidence ?? 0) >= 2,
  ).length;
  const highFromL3 = auraImpacts.some(
    (i) => (i.level ?? 0) >= 3 && (i.confidence ?? 0) >= 3,
  );
  ...
  return [
    ...
    {
      reason: 'Score of 150M+',
      status: score >= 150_000_000 ? 'passed' : 'incomplete',
      level: 4,
    },
    {
      reason:
...
export const getLevelupProgress = async ({
  ...
  const profileQuery = await getSubjectVerifications(
    subjectId,
    evaluationCategory,
  );
  ...
  if (evaluationCategory !== EvaluationCategory.SUBJECT) {
    throw new Error(
      'Evaluation category is only supported for subject for now',
    );
  }
