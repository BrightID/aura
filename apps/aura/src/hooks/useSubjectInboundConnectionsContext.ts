...
export function useSubjectInboundConnectionsSetup(subjectId: string | undefined) {
...
export function useSubjectInboundConnectionsContext(props: { subjectId: string; evaluationCategory?: EvaluationCategory }) {
  ...
  if (!data) throw new Error(`useSubjectInboundConnectionsSetup for ${props.subjectId} not called in a parent component`);
