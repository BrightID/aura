  ...
  try {
    ...
    if (user && !useUserStore.getState().id) {
      useUserStore.setState({
        ...
        updateTimestamps: (user.updateTimestamps as UserState['updateTimestamps']) ?? {
          backupCompleted: 0, isSponsored: 0, isSponsoredv6: 0, photo: 0, name: 0, password: 0,
        },
    ...
    if (profile && !useProfileStore.getState().authData) {
      useProfileStore.setState({
        authData: profile.authData as { brightId: string; password: string } | null,
        brightIdBackupEncrypted: profile.brightIdBackupEncrypted as string | null,
        splashScreenShown: (profile.splashScreenShown as boolean) ?? false,
        playerOnboardingScreenShown: (profile.playerOnboardingScreenShown as boolean) ?? false,
      });
    }
    ...
    if (contacts?.storedInfoHashed) {
      useContactsStore.setState({
        storedInfoHashed: contacts.storedInfoHashed as { type: string; value: string }[],
      });
    }
