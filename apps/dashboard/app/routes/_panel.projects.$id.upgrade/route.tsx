...
export default function PricingSection() {
  ...
  const currentPlan = plans.find(
    (item) => focusedProject?.selectedPlanId === item.id,
  );
      ...
      try {
        ...
        if (data.status === 'completed') {
          ...
          await axios.post(
            ...
            {
              planId: activeWidget.planId,
              projectId: focusedProject!.id,
              orderId: activeWidget.orderId,
              isYearly,
            },
        ...
        <div className="mt-10 text-center space-y-6">
          ...
          <div className="flex items-center justify-center gap-8 text-sm text-muted-foreground">
            <span className="flex items-center gap-2">
              <Check className="h-4 w-4 text-primary" /> No hidden fees
            </span>
            <span className="flex items-center gap-2">
        ...
        <div slot="content" className="max-w-2xl w-full p-0 overflow-hidden">
          ...
          {paymentStatus === 'failed' && (
            <div className="flex flex-col items-center gap-4 py-10 px-6">
              ...
              <a-button variant="secondary" onClick={handleClose}>
                Close
              </a-button>
            </div>
          )}
