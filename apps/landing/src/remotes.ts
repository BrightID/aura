...
async function installReactRefreshPreamble(url: string) {
  ...
  const inject = runtime.injectIntoGlobalHook ?? runtime.default?.injectIntoGlobalHook
  inject?.(window)
  w.$RefreshReg$ = () => {}
  ...
  (name): { name: RemoteName; prefix: string; load: () => Promise<MountModule> } => {
    ...
    return {
      ...
      load: async () => {
        ...
        return import(/* @vite-ignore */ import.meta.env.DEV ? dev : prod) as Promise<MountModule>
      },
    }
  },
)
